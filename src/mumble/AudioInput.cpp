...
void AudioInput::resetAudioProcessor() {
	...
	if (iEchoChannels > 0) {
		...
		if (Global::get().s.echoOption == EchoCancelOptionID::WEBRTC_AEC) {
			...
			m_apm = webrtc::BuiltinAudioProcessingBuilder().SetConfig(cfg).Build(webrtc::CreateEnvironment());
			if (m_apm) {
...
void AudioInput::updateVad(Settings::VADSource src) {
	...
	if (src == Settings::WebRTC) {
		...
		storeWebrtcVad(std::move(u));
	} else {
		storeWebrtcVad(nullptr);
	}
#endif
	m_vad.store(src);
}
...
void AudioInput::updateWebrtcAggressiveness(webrtc::Vad::Aggressiveness aggressiveness) {
	...
	if (m_vad.load() == Settings::WebRTC && loadWebrtcVad()) {
		auto u = webrtc::CreateVad(m_vadWebrtcAggressiveness);
		if (!u) {
			qWarning() << "AudioInput: Failed to initialize WebRTC VAD, disabled";
		}
