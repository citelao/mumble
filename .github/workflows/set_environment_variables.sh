...
if [[ "$build_type" == "static" ]]; then
	...
	# webrtc-audio-processing needs vcpkg's unofficial-webrtc target, which only static builds
	# have access to (shared builds use system packages, not vcpkg).
	ADDITIONAL_CMAKE_OPTIONS="$ADDITIONAL_CMAKE_OPTIONS -Dwebrtc-audio-processing=ON"
